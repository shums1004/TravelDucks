...
class Weather extends Component {
  ...
  componentDidMount() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        //Weather Api call
        Axios.get(
          `http://api.weatherstack.com/current?access_key=006d71ec3fd4efa58ea8e390ebb26fa9&query=${this.state.userPosition.latitude},${this.state.userPosition.longitude}`
        ).then((res) => {
          let userWeather = {
  ...
  changeLocation = (e) => {
    ...
    Axios.get(
      `http://api.weatherstack.com/current?access_key=006d71ec3fd4efa58ea8e390ebb26fa9&query=${this.state.regionInput}`
    ).then((res) => {
      let userWeather = {
        temperature: res.data.current.temperature,
        description: res.data.current.weather_descriptions[0],
