...
const LocationList = ({ classes, data }) => {
  ...
  return (
    <div className="LocationList__container">
      <SearchFilter />
      <Typography className={classes.results} variant="button">
        {filteredVenues.length} results for {detail} in {location}{" "}
      </Typography>
      <MenuList className={classes.list}>
        {filteredVenues.map((venue) => (
          <MenuItem
            ...
            key={venue.id}
            className={
              isCurrentLocation(data.currentLocation, venue)
                ? classes.currentVenue + " " + classes.menuItem
                : classes.menuItem
            }
          >
            <ListItemText
